...
export interface SecurityPlayerPersonalDataSummary {
  playerPublicId: string;
  hasRow: boolean;
...
export async function fetchSecurityPlayerPersonalData(
  playerId: string,
): Promise<SecurityPlayerPersonalDataSummary> {
  const data = await securityData(
    `/api/security/players/${encodeURIComponent(playerId)}/personal-data`,
  );
  const rec = asRecord(data);
  return {
    playerPublicId: str(rec.player_public_id ?? rec.playerPublicId, playerId),
