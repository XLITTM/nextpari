      ...
      {helpOpen && (
        <Drawer title="Правила 21 / Очко" onClose={() => setHelpOpen(false)}>
          <ul className="space-y-2 text-sm leading-relaxed text-white/80">
            ...
            <li>Две карты банкира открыты с начала раунда, очки считаются сразу по обеим.</li>
            <li>Два туза — золотое очко, мгновенная победа.</li>
            <li>При равенстве очков побеждает банкир.</li>
            <li>Банкир добирает карты, пока не достигнет очков игрока или не получит 21/перебор, и не останавливается раньше 17.</li>
            <li>Выплата при обычной победе — ×2.00 от ставки. Золотое очко — ×2.00.</li>
          </ul>
        </Drawer>
      )}
