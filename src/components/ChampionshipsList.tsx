...
export function ChampionshipsList() {
  ...
  const leagues = Object.values(
    liveMatches.reduce<Record<string, { name: string; country: string; count: number }>>((acc, match) => {
      const key = `${match.country}|${match.league}`;
      const prev = acc[key] ?? { name: match.league, country: match.country, count: 0 };
      prev.count += 1;
      acc[key] = prev;
      return acc;
    }, {}),
  ...
  return (
    <div>
      ...
      <div className="px-4 space-y-2">
        {leagues.length === 0 ? (
          <p className="text-sm font-bold text-gray-600 dark:text-gray-300 py-6 text-center">
            Живые чемпионаты появятся после загрузки ленты
          </p>
        ) : (
          leagues.map((ch) => (
            <div
            ...
            >
              <div className="relative shrink-0">
                <div
                  ...
                  style={{ backgroundColor: colorFromName(ch.name) + '30' }}
                >
                  <div className="w-5 h-5 rounded-full" style={{ backgroundColor: colorFromName(ch.name) }} />
                </div>
