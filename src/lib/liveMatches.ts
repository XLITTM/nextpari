...
async function fetchLiveMatchesNested(): Promise<MatchEvent[] | null> {
  ...
  if (error) {
    console.error('Failed to load live matches:', error.message);
    return null;
  }
...
async function hydrateMatchRows(matches: (MatchRow & { tournament_id?: string })[]): Promise<MatchEvent[]> {
  const tournamentIds = [...new Set(matches.map((row) => row.tournament_id).filter(Boolean))];
  const matchIds = matches.map((row) => row.id);

  const [{ data: tournaments }, { data: markets }] = await Promise.all([
    tournamentIds.length
      ? supabase.from('tournaments').select('*').in('id', tournamentIds)
      : Promise.resolve({ data: [] as TournamentRow[] }),
    matchIds.length
      ? supabase.from('markets').select('*').in('match_id', matchIds)
      : Promise.resolve({ data: [] as MarketRow[] }),
  ]);

  const marketRows = (markets ?? []) as (MarketRow & { match_id?: string })[];
  const marketIds = marketRows.map((row) => row.id).filter(Boolean) as string[];
  const { data: odds } = marketIds.length
    ? await supabase.from('odds').select('*').in('market_id', marketIds)
    : { data: [] as (OddRow & { market_id?: string })[] };

  const oddsByMarket = new Map<string, OddRow[]>();
  for (const odd of (odds ?? []) as (OddRow & { market_id?: string })[]) {
    if (!odd.market_id) continue;
    const list = oddsByMarket.get(odd.market_id) ?? [];
    list.push(odd);
    oddsByMarket.set(odd.market_id, list);
  }

  const marketsByMatch = new Map<string, MarketRow[]>();
  for (const market of marketRows) {
    if (!market.match_id || !market.id) continue;
    const list = marketsByMatch.get(market.match_id) ?? [];
    list.push({ ...market, odds: oddsByMarket.get(market.id) ?? [] });
    marketsByMatch.set(market.match_id, list);
  }

  const tournamentById = new Map(
    ((tournaments ?? []) as (TournamentRow & { id?: string })[])
      .filter((row) => row.id)
  ...
  return matches.map((row) =>
    mapMatch({
      ...row,
      tournaments: row.tournament_id ? tournamentById.get(row.tournament_id) ?? null : row.tournaments,
      markets: marketsByMatch.get(row.id) ?? row.markets ?? [],
    }),
  );
}

async function fetchLiveMatchesFlat(): Promise<MatchEvent[]> {
  ...
  if (error || !matches) {
    if (error) console.error('Failed to load live matches:', error.message);
    return [];
  }

  return hydrateMatchRows(matches as (MatchRow & { tournament_id?: string })[]);
}

export async function fetchLiveMatches(): Promise<MatchEvent[]> {
  const nested = await fetchLiveMatchesNested();
  if (nested) return sortCatalog(nested);
  return sortCatalog(await fetchLiveMatchesFlat());
}
...
export async function fetchUpcomingMatches(): Promise<MatchEvent[]> {
  const nested = await supabase
    ...
    .eq('is_live', false)
    .gte('start_time', upcomingSince())
    .order('featured', { ascending: false })
    .order('start_time', { ascending: true })
    .limit(100);

  if (!nested.error && nested.data) {
    return sortCatalog(((nested.data ?? []) as MatchRow[]).filter((row) => !isFinishedMatch(row)).map(mapMatch));
  }
  ...
  const { data: matches, error } = await supabase
    ...
    .limit(100);
  if (error || !matches) {
    if (error) console.error('Failed to load upcoming matches:', error.message);
    return [];
  }
  return sortCatalog(
    await hydrateMatchRows((matches as (MatchRow & { tournament_id?: string })[]).filter((row) => !isFinishedMatch(row))),
  );
}
