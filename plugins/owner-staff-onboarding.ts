...
import { attachBetConstructHttp } from '../server/betconstruct/http';
...
function attachOwnerStaff(server: ViteDevServer) {
  server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
    void attachOwnerAuthHttp(req, res)
      ...
      .then((handled) => {
        if (handled) return true;
        return attachPlayerGamesHttp(req, res);
      })
      .then((handled) => {
        if (handled) return true;
      ...
      .then((handled) => {
        if (handled) return true;
        return attachBetConstructHttp(req, res);
      })
      .then((handled) => {
        if (handled) return true;
      ...
      .then((handled) => {
        if (handled) return true;
        return attachSportsSettleHttp(req, res);
      })
      .then((handled) => {
        if (handled) return true;
